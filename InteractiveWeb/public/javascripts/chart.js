...
function DrawSortedLegend(args,reqproms,multikey,anyFaded) {
    ...
    let legData = ArgsArr.map(function(a,i) {
        let xrank = (anyFaded) ? ArgsArr.indexOf(a) : ArgsArr.indexOf(a)+1;
        let xid = reqproms[i].request[multikey];
    return {
        // index:a[0].data.interindexes.slice(-1)[0],
        index:a[0].rows.sort((a,b) => (a.period > b.period) ? 1 : -1).slice(-1)[0],

        // text:a[0].data[multikey][0].name,
        text:controllers[multikey].results.find(group => group.children.some(item => item.id === xid)).children.find(x=>x.id === xid).legname,
        rank: xrank,
...
function DrawLine(result,rank,request,multikey) {
    ...
    function filterResults (yr) {
            ...
            el = result.rows.find(x=> x.period === yr);
    ...
    lines.append('path')
        ...
        .on('mouseover', function (d) {
            ...
            let legname = controllers[multikey].results.find(group => group.children.some(item => item.id === xid)).children.find(x=>x.id === xid).legname;
